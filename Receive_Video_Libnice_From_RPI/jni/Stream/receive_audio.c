...
{
	GstElement *pipeline, *nicesrc, *capsfilter,
				*rtpspeexdepay, *speexdec, *autoaudiosink;
	GstBus *bus;
	GstMessage *msg;
	...
	g_object_set (capsfilter, "caps",
				  gst_caps_from_string("application/x-rtp, "
						  ...
						  "timestamp-offset=(uint)2918479805, "
						  "seqnum-offset=(uint)26294"), NULL);

	pipeline = gst_pipeline_new ("test-pipeline");
	if (!pipeline || !nicesrc || !capsfilter ||
			!rtpspeexdepay|| !speexdec|| !autoaudiosink)
	{
	...
	if (gst_element_link_many ( nicesrc, capsfilter, rtpspeexdepay,
			speexdec, autoaudiosink, NULL) != TRUE)
	{
