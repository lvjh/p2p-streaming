...
#if GST_CHECK_VERSION (1,0,0)
  __android_log_print(ANDROID_LOG_ERROR, "TAG", "nice_agent_send 1");
  GstMapInfo info;
...
#else
  int ret = nice_agent_send (nicesink->agent, nicesink->stream_id,
      nicesink->component_id, GST_BUFFER_SIZE (buffer),
      (gchar *) GST_BUFFER_DATA (buffer));

  //__android_log_print(ANDROID_LOG_ERROR, "TAG", "agent = %d, stream_id = %d, component_id = %d", nicesink->agent, nicesink->stream_id, nicesink->component_id);
 // __android_log_print(ANDROID_LOG_ERROR, "TAG", "value = %d", ret);
